...
\begin{noteblock}
  This is version~0.2 of \pkg{absfigure}. The package is functional
...
the float is never placed and a warning is raised at the end of the
\LaTeX{} run.

\begin{warningblock}
  If a float targets a page that has already been shipped out in the
  current run, it cannot be placed until the next run. In that case,
  rerun \LaTeX{} after editing. The package writes a placement cache
  and emits a warning when absolute-float placement is not yet final.
\end{warningblock}
...
\item[\key{page} exceeds total pages] %
  \textbf{Warning} at end of run:
  \texttt{Float N (figure/table, target page=X) was not placed in this run}.
  If the warning persists after rerunning \LaTeX{}, the target page or
  slot is unreachable.
...
\item[float targets an earlier page than its position in the source] %
  Valid. Rerun \LaTeX{} so the cached float definition can be replayed
...
If multiple absolute floats target the same slot (\key{page},
\key{col}, and \key{pos}), they are placed in source order. A later
float does not overtake an earlier one in the same slot.
...
\begin{itemize}
\item If a float targets a page that is already past in the current
  run, an additional \LaTeX{} run is needed to finalize placement.
  This is expected and works similarly to other multi-pass features
  such as cross-references and bibliographies.
...
\item[v0.1 (2026/02/25)] Initial release.
  ...
  \end{itemize}
\item[v0.2 (2026-04-03)] Documentation and placement update.
  \begin{itemize}
  \item Environments documented with optional-argument syntax
  \item Earlier-page placement works across reruns via a cache file
  \item End-of-run warning now tells the user to rerun \LaTeX{}
  \item Floats targeting the same slot are placed in source order
  \end{itemize}
\end{description}
